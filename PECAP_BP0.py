...
#          console and confirm that all parameters are as desired before moving onto step 5
...
# This code also allows for the current spread to be artificially decreased next to one electrode in BP0 mode.

########################################
# Charlotte Garcia, November 2019
...
def NIC2_ECAP_FM(probe_elec, probe_level, masker_elec, masker_level, rec_elec, params):
    ...
    BP0_elec = params['BP0_elec']       # the electrode that will be stimulated in BP mode to reduce current spread
    ...
    # error checking for BP0 electrode grounding
    if (BP0_elec < 3) or (BP0_elec > 20):
        print 'Electrode stimualted in Bipolar more (BP0_elec) cannot be at the edges of the array - exiting.'
        return

    # define ground for probe electrode
    if BP0_elec == probe_elec:
        if BP0_elec - 1 == rec_elec:
            print 'rec elec needs to change to accomodate probe_ground'
            p_ground = BP0_elec - 1
        else:
            p_ground = BP0_elec - 1
    else:
        p_ground = nic.Channel.ECE1

    # define ground for masker electrode
    if BP0_elec == masker_elec:
        if BP0_elec - 1 == rec_elec:
            if BP0_elec == 20:
                print 'To accommodate Masker Ground, the recording electrode has been moved from e%d to e%d.' % (rec_elec, rec_elec + 3)
                rec_elec = rec_elec + 3
            else:
                print 'To accommodate Masker Ground, the recording electrode has been moved from e%d to e%d.' % (rec_elec, rec_elec + 2)
                rec_elec = rec_elec + 2
            m_ground = BP0_elec - 1
        else:
            m_ground = BP0_elec - 1
    else:
        m_ground = nic.Channel.ECE1

    # print the probe/masker/recording and ground scenarios for visualization purposes
    print 'Probe:', probe_elec, 'Level:', probe_level, 'Masker:', masker_elec, 'Level:', masker_level, 'Record:', rec_elec
    print 'Probe Ground:', p_ground, 'Masker Ground:', m_ground
    ...
    stim_masker0  = nic.BiphasicStimulus(masker_elec, m_ground, 0,            pw, ipg,  120.0)		         # 120 = 400 - 280 (280 taken by 4 * 70 us)
    stim_probe    = nic.BiphasicStimulus(probe_elec,  p_ground, probe_level,  pw, ipg, 2000.0)
    stim_masker   = nic.BiphasicStimulus(masker_elec, m_ground, masker_level, pw, ipg,  120.0)               # changing masker to equal probe level (instead of probe+10)
    stim_probe0   = nic.BiphasicStimulus(probe_elec,  p_ground, 0,            pw, ipg, 2000.0)
    ...
    for i in range(0,reps):
        ...
        ECAP[i+1] = {'Probe':probe_elec,
                   'Masker':masker_elec,
                   'Recording': rec_elec,
                   'ProbeGround': p_ground,
                   'MaskerGround': m_ground,
...
def generate_nrt_order_fm(elecs, e_levels, record_from, super_elecs, diagrecelec, params):
    ...
    if len(super_elecs) > 0:
        print 'Super Electrodes not programmed as an option for BP0 simulation mode - exiting.'
        return
    for i in range(0, len(diagrecelec)):
        if (diagrecelec[i] == 1) or (diagrecelec[i] == -1):
            print 'Cannot record 1 electrode away from the diagonal for BP0 simulation mode - exiting.'
            return
    if params['BP0_elec'] < elecs[0] or params['BP0_elec'] > elecs[1]:
        print 'Electrode stimulated in Bipolar mode cannot be outside stimulated electrode range - exiting.'
        return
                    ...
                    for i in range(0, len(diagrecelec)):
                        if (p + diagrecelec[i]) >= min_elec and (p + diagrecelec[i]) < max_elec + 1:
                            ...
                            NRT_run_order[nrtno] = {'Probe Electrode': p, 'Probe Level': e_levels[p-min(elecs)],
                                                    'Masker Electrode': m, 'Masker Level': e_levels[m-min(elecs)],
                                                    'Recording Electrode': r}
                        # for the case where we are only using one electrode to record along the diagonal
                        elif len(diagrecelec) == 1:
                            # if we are recording basally, and are at the basal end of the array
                            if (diagrecelec[i] == -2) and (p + diagrecelec[i] < 1):
                                nrtno = nrtno + 1
                                r = p - diagrecelec[i]
                                NRT_run_order[nrtno] = {'Probe Electrode': p, 'Probe Level': e_levels[p - min(elecs)],
                                                        'Masker Electrode': m, 'Masker Level': e_levels[m - min(elecs)],
                                                        'Recording Electrode': r}
                            # if we are recording apically, and are at the apical end of the array
                            elif (diagrecelec[i] == 2) and (p + diagrecelec[i] > 22):
                                nrtno = nrtno + 1
                                r = p - diagrecelec[i]
                                NRT_run_order[nrtno] = {'Probe Electrode': p, 'Probe Level': e_levels[p - min(elecs)],
                                                        'Masker Electrode': m, 'Masker Level': e_levels[m - min(elecs)],
                    ...
                    for ridx in range(min_elec, max_elec + 1):
                        ...
                        if (ridx != p) and (ridx != m):
                            ...
                            NRT_run_order[nrtno] = {'Probe Electrode': p, 'Probe Level': e_levels[p - min(elecs)],
                                                    'Masker Electrode': m, 'Masker Level': e_levels[m - min(elecs)],
                                                    'Recording Electrode': r}
                # if probe is on electrodes 1:20 (assuming our default is to record 2 apically)
                elif p <= (max_elec - record_from):
                    if (p + record_from) != m:
                        nrtno = nrtno + 1
                        r = p + record_from
                        NRT_run_order[nrtno] = {'Probe Electrode': p, 'Probe Level': e_levels[p - min(elecs)],
                        ...
                        NRT_run_order[nrtno] = {'Probe Electrode': p, 'Probe Level': e_levels[p - min(elecs)],
                                                'Masker Electrode': m, 'Masker Level': e_levels[m - min(elecs)],
                                                'Recording Electrode': r}
                # if probe is on electrodes 21:22 (assuming our default is to record 2 apically)
                else:
                    if (p - record_from) != m:
                        nrtno = nrtno + 1
                        r = p - record_from
    ...
    print 'Electrode', params['BP0_elec'], 'is being stimulated in BP-0 mode with Electrode', params['BP0_elec'] - 1
...
def gen_csv_fm(filename,NRTs,nrtno,params,elecs,e_levels):
    ...
    fp.write('BP0 Electrode,BP0 Electrode Ground\n')
    ...
    fp.write(str(params['BP0_elec']));                  fp.write(',')
    fp.write(str(params['BP0_elec'] - 1));              fp.write(',')
    fp.write('\n\n')
    ...
    for nrt in range(0,nrtno):
        for rep in range(0,params['reps']):
            fp.write('NRT,Repetition,Probe Electrode,Masker Electrode,Recording Electrode,Probe Ground,Masker Ground\n')
            ...
            fp.write(str(NRTs[nrt+1][rep+1]['ProbeGround']));   fp.write(',')
            fp.write(str(NRTs[nrt+1][rep+1]['MaskerGround']));  fp.write('\n')
            # write A
            fp.write('A,')
...
#elecs = [3,21]
#e_levels = [138, 140, 140, 142, 144, 144, 146, 143, 139, 136, 136, 136, 136, 133, 129, 126, 121, 116]
elecs = [1, 22]
e_levels = [201,202,203,204,205,206,207,208,209,210, 211, 212, 213, 214, 215, 216, 217,218,219,220,221,222]
...
diagrecelec = [2]     # electrodes to record from along the diagonal (i.e. p = m)
params = {'gain':       50,     # in dB
          'delay':      98,     # in us
          'ipg':        8.0,    # in us
          ...
          'BP0_elec':   13}     # electrode used for BP-1 stimulation, to simulate lower current spread
filename_fm = 'TEST_PECAP_NIC2_BP0_18May21.csv'           # name of csv filename that you'd like to save this data to for this subject
