...
# This script is for collecting PECAP using NIC2 for Cochlear subjects with dead zone simulation
# It can be used as an alternative to Custom Sound EP, and allows the flexibility to record multiple repetitions of the
#   same parameters (up to 4 at a Probe Rate of 80Hz) with no additional time. Recording 12 sweeps 4 times in Custom
#   Sound EP requires approximately 4 seconds per NRT (i.e. 4 *4 = 16), whereas in NIC2 this takes approximately 5
#   seconds.
...
#   data acquired *so far* to a csv file after the fact from within the PyCharm environment
# This Program is an alteration of the original NIC2 PECAP data collection script, as it simulates a dead region
#   centered on one electrode by sending signals at a rate fast enough to keep the neurons near that electrode in a
#   constant refractory state prior to recording of any of the frames of the ECAP
# Method for usage:
...
def NIC2_ECAP_FM(probe_elec, probe_level, masker_elec, masker_level, rec_elec, params):
    ...
    #       'dead_elec' = electrode to simulate dead region on
    #       'dead_level' = level of the pulses presented on the dead electrode (different level & pps than PECAP MCL)
    ...
    if params['reps'] == 5:
        ...
        print 'Running at 350Hz, not running dead region simulation.'
    else:
        nrt_pup_seq = nic.Sequence(20)
        nrt_pup_seq.append(nic.StimulusCommand(nic.NullStimulus(465)))
        nrt_dog_seq = nic.Sequence(2)
        stim_dog = nic.BiphasicStimulus(params['dead_elec'], nic.Channel.ECE1, params['dead_level'], pw, ipg, 400.0)
        nrt_dog_seq.append(nic.StimulusCommand(stim_dog))
    ...
    for i in range(0,reps):
        ...
        measurementSeq.append(nrt_pup_seq)		# lasts 25*388 = 9700 us
        measurementSeq.append(nrt_dog_seq)      # lasts 400 us (total = 10100 us)
                    ...
                    for i in range(min_elec, max_elec + 1):
                        if p != i:
                            ...
                            NRT_run_order[nrtno] = {'Probe Electrode': p, 'Probe Level': e_levels[p-min(elecs)],
                                                    'Masker Electrode': m, 'Masker Level': e_levels[m-min(elecs)],
                                                    'Recording Electrode': r}
                            print nrtno, NRT_run_order[nrtno]
                # for loop for multiple recording electrodes along the diagonal, non-super electrodes
                else:
                    for i in range(0, len(diagrecelec)):
                        if (p + diagrecelec[i]) >= min_elec and (p + diagrecelec[i]) < max_elec + 1:
...
def gen_csv_fm(filename,NRTs,nrtno,params,elecs,e_levels):
    ...
    fp.write('Dead Region Electrode,Dead Region Level\n')
    ...
    fp.write(str(params['dead_elec']));                 fp.write(',')
    fp.write(str(params['dead_level']));                fp.write(' CUs,')
    fp.write('\n\n')
...
# left ear
elecs = [1,22]
e_levels = [216, 205, 193, 182, 180, 178, 176, 177, 177, 178, 175, 173, 170, 168, 166, 164, 168, 170, 174, 170, 166, 162]
#e_levels = [212, 200, 186, 174, 172, 172, 170, 170, 170, 170, 167, 163, 160, 158, 158, 156, 159, 161, 164, 162, 160, 158]
...
diagrecelec = [-2, 2]     # electrodes to record from along the diagonal (i.e. p = m)
params = {'gain':       60,   # in dB
          'delay':      73,   # in us
          'ipg':        8.0,    # in us
          ...
          'PD':         25.0,    # phase duration in us
          'dead_elec':  14,      # electrode on which a dead region is simulated by sending pulses to keep in refractory
          'dead_level': 168}    # assigns the level of the dead electrode simulation pulses
filename_fm = 'C30_PECAP_NIC2_DeadRegion_Mar20.csv'           # name of csv filename that you'd like to save this data to for this subject
